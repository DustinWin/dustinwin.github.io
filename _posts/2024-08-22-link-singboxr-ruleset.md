...
如：[ShellCrash](https://github.com/juewuy/ShellCrash) 和 [sing-boxr for Android](https://github.com/reF1nd/sing-box-releases/releases) 等
...
3. 本教程必须使用支持[提供者](https://sing-boxr.dustinwin.cc.cd/zh/configuration/provider/) `outbound_providers`（类似于 [mihomo 内核](https://github.com/MetaCubeX/mihomo)的[代理集合](https://wiki.metacubex.one/config/proxy-providers/) `proxy-providers`）的 [sing-box reF1nd 版内核](https://github.com/reF1nd/sing-box)，请先**确定自己机场的订阅链接是否为 Clash 或 sing-box 订阅链接**，若不是，需前往[肥羊在线订阅转换工具](https://suburl.v1.mk)进行转换，“生成类型”选择“Clash”或“sing-box”，其它参数保持默认即可，转换后的 Clash 订阅链接需要在末尾添加 `&flag=clash`，然后添加到 .json 文件出站提供者 `providers` 的 `url` 中
4. 推荐使用 [Visual Studio Code](https://code.visualstudio.com/Download) 等专业编辑器来修改配置文件
5. ShellCrash 支持本地导入配置文件，可以直接将下方的 .json 直链文件内容复制到 `$CRASHDIR/jsons/config.json`{: .filepath} 文件中，可代替通过 ShellCrash 配置脚本 → 6) 配置文件管理 → a) 添加提供者
...
```json
{
  ...
  "route": {
    ...
    "rules": [
      ...
      { "action": "sniff" },
      // 若使用 ShellCrash，会自动覆写此条，可删除此条 `hijack-dns`
    ...
    // 规则集（binary 文件每天自动更新）
    "rule_set": [
      // 若使用 ShellCrash，由于无法判断本机进程（默认删除 `process_name` 规则），需删除 `applications`
      {
        "tag": [ "ads", "private", "applications", "microsoft-cn", "apple-cn", "google-cn", "games-cn", "games", "media", "ai", "networktest", "proxy", "cn", "privateip", "telegramip", "cnip", "mediaip" ],
        "type": "remote",
        "format": "binary",
...
```json
{
  ...
  "route": {
    ...
    "rule_set": [
      {
        "tag": [ "ads", "private", "games", "media", "ai", "networktest", "tld-proxy", "gfw", "telegramip", "mediaip" ],
        "type": "remote",
        "format": "binary",
        "path": "./ruleset/{tag}.srs",
        "url": "https://github.com/DustinWin/ruleset_geodata/releases/download/sing-box-ruleset/{tag}.srs",
...
```json
{
  ...
  "route": {
    ...
    "rule_set": [
      {
        "tag": [ "bilibili", "netflix", "netflixip" ],
        "type": "remote",
        "format": "binary",
        "path": "./ruleset/{tag}.srs",
        "url": "https://github.com/DustinWin/ruleset_geodata/releases/download/sing-box-ruleset/{tag}.srs"
